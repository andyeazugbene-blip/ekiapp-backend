...
export async function adminResetUsers(_req: Request, res: Response): Promise<void> {
  ...
  // Create vendor@eki.app (dual-role)
  ...
  // Create buyer@eki.app
  await prisma.user.create({
    data: { email: "buyer@eki.app", name: "Amara Buyer", password: hash, phone: "+447700900222", country: "United Kingdom", role: "BUYER", emailVerifiedAt: new Date(), referralCode: "BUYERAMARA10", trustScore: 50 },
  });

  // Create admin@eki.app
  await prisma.user.create({
    data: { email: "admin@eki.app", name: "Admin", password: hash, role: "ADMIN", emailVerifiedAt: new Date() },
  });
